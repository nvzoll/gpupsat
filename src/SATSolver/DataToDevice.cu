...
)
{
    ...
    d_queue = thrust::raw_pointer_cast(thrust::device_malloc<JobsQueue>(1));
    check(cudaMemcpy(d_queue, &queue, sizeof(JobsQueue), cudaMemcpyHostToDevice), "Copying JobsQueue object to device");

    d_clauses_db = thrust::device_malloc<CUDAClauseVec>(1).get();
    check(cudaMemcpy(d_clauses_db, &clauses_db, sizeof(CUDAClauseVec), cudaMemcpyHostToDevice),
        "Copying CUDAClauseVec object to device");
}
...
__device__ GPUVec<Lit> *get_assumptions_sequential() { return assumptions_sequential; }
#endif

__host__ __device__ JobsQueue& DataToDevice::get_jobs_queue() { return queue; }

__host__ __device__ JobsQueue *DataToDevice::get_jobs_queue_ptr() { return &queue; }

__host__ __device__ CUDAClauseVec const& DataToDevice::get_clauses_db() { return clauses_db; }
__host__ __device__ int DataToDevice::get_n_vars() { return n_vars; }
...
__host__ __device__ RuntimeStatistics *DataToDevice::get_statistics_ptr() { return &statistics; }

__host__ __device__ unsigned int *DataToDevice::get_found_answer_ptr() { return found_answer; }

__host__ __device__ Results DataToDevice::get_results() { return results; }

__host__ __device__ Results *DataToDevice::get_results_ptr() { return &results; }

__host__ __device__ NodesRepository<GPULinkedList<WatchedClause *>::Node> *DataToDevice::get_nodes_repository_ptr()
{
