...
program_options_t get_program_options(int argc, char *argv[])
{
    ...
    ("number-of-threads,t"          , po::value<int>()           ,  "Number of threads")
    ("number-of-blocks,b"           , po::value<int>()           ,  "Number of blocks")
    ("verbosity-level,v"            , po::value<int>()           ,  "Set verbosity level")
    ("strategy,s"                   , po::value<std::string>()   ,
     "Strategy for generating the jobs: distributed or uniform")
    ("sequential-as-parallel,p"                                  ,
     "Forces the execution of the parallel strategy, with jobs creation, but running with 1 thread and 1 block")
    ("preprocess-unary-clauses,u"                                ,  "Turns on the pre-processing of unary clauses")
    ("write-log,l"                                        ,  "Prints file,threads,blocks,ms to autolog.txt")
    ;
