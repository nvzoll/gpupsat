    ...
    : structure(n_vars, handler, node_repository)
{
    if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
        printf("WatchedClausesList constructor: n_vars=%d, handler=%p, node_repository=%p, n_clauses=%d\n",
            n_vars, handler, node_repository, n_clauses);
    }
    this->vars_handler = handler;
    this->next_watched_clause_index = 0;
    int repo_size = n_clauses + 1
#ifdef USE_CLAUSE_LEARNING
        + MAX_LEARNT_CLAUSES_PER_THREAD
#endif
        ;
    if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
        printf("  Attempting to allocate repository size: %d\n", repo_size);
    }
    this->repository = new GPUVec<WatchedClause>(repo_size); // repository;
    if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
        printf("  Repository allocated: ptr=%p\n", this->repository);
    }
    for (int i = 0; i < repo_size; i++) {
        if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x) && i % 1000 == 0) { // Print every 1000 iterations
            printf("  Initializing repository index %d\n", i);
        }
        WatchedClause wc;
        // wc.clause = nullptr; // Clause is a struct, not a pointer here
        wc.watched_lit_index_1 = -1;
        wc.watched_lit_index_2 = -1;
        this->repository->add(wc); // Assuming add handles potential reallocation if needed
    }
    if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
        printf("WatchedClausesList constructor finished initialization loop.\n");
    }
}
    ...
    GPULinkedList<found_implication>& implication_list)
{
    ...
    GPULinkedList<WatchedClause*>::LinkedListIterator iter = structure.get_iterator(var(decision.literal));
    ...
    GPULinkedList<WatchedClause*>::LinkedListIterator temp_iter = temporary_clauses.get_iterator();
    ...
    GPULinkedList<found_implication>::LinkedListIterator iter2 = implication_list.get_iterator(last_implications_size);
    ...
    GPULinkedList<found_implication>& implication_list)
{
    ...
    Lit first_watched_literal = watched_clause->clause.literals[watched_clause->watched_lit_index_1];

    Lit second_watched_literal = watched_clause->clause.literals[watched_clause->watched_lit_index_2];

#ifdef USE_ASSERTIONS
    assert(var(decision.literal) == var(first_watched_literal) || var(decision.literal) == var(second_watched_literal));
#endif
...
{
    if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
        printf("add_all_clauses: formula size = %d\n", formula.size_of());
    }
    for (int i = 0; i < formula.size_of(); i++) {
        const Clause* c = formula.get_ptr(i);
        if (c->n_lits > 1) {
            if (DEBUG_SHOULD_PRINT(threadIdx.x, blockIdx.x)) {
                printf("  Processing clause %d: ptr=%p, n_lits=%u, literals_ptr=%p\n", i, c, c->n_lits, c->literals);
            }
            new_clause(c);
        }
    }
